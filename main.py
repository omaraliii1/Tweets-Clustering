...
def k_means(tweets, k=4, max_iterations=50):
    ...
    hash_map = dict()
    while count < k:
        random_tweet_idx = rd.randint(0, len(tweets) - 1) # get random centroid
...
def assign_cluster(tweets, centroids):
    ...
    # for every tweet iterate each centroid and assign closest centroid to it
    for t in range(len(tweets)): 
        ...
        clusterID=clusters.setdefault(cluster_idx, [])
        
        clusterID[last_tweet_idx].append(min_dis)
